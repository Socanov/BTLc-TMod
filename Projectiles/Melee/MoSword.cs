...
namespace BTLc.Projectiles.Melee
{
    public class MoSword : BTLcProj
    {
		...
		public override void SetDefaults() {
			...
			Projectile.extraUpdates = 1; // 额外更新 每次更新 1+额外更新 次
		}
		...
		public override void AI() {
			...
			if(player.direction > 0){
				Projectile.position.X = player.position.X + player.direction * 20;//+ Vector2.Normalize(Projectile.velocity);//  * (Timer - 1f);
			}
			else
			{
				Projectile.position.X = player.position.X - (player.direction * 20 + Projectile.width / 2 + 30) ;//
			}
